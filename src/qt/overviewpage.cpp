...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(CampusCashUnits::CCASH)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = CampusCashUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(CampusCashUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(CampusCashUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(CampusCashUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(CampusCashUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(CampusCashUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    //ui->labelWatchAvailable->setText(CampusCashUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    //ui->labelWatchStake->setText(CampusCashUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    //ui->labelWatchPending->setText(CampusCashUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    //ui->labelWatchImmature->setText(CampusCashUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    //ui->labelWatchTotal->setText(CampusCashUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelCCASHtotalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
    ...
    if(stat)
    {
        ...
        if(nSyncPercentage != 100)
        {
            if(clientModel->getNumConnections() > 0)
                ui->clStat->setText("<font color=#fad745>" + QSyncing + "</font>");
        }
        return;
    }
}
...
{
    CampusCashGUI::gotoHistoryPage_static();
}
