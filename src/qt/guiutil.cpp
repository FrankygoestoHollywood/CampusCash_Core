...
{
    widget->setMaxLength(CampusCashAddressValidator::MaxAddressLength);
    widget->setValidator(new CampusCashAddressValidator(parent));
...
bool parseCampusCashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("CampusCash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CampusCashUnits::parse(CampusCashUnits::CCASH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCampusCashURI(QString uri, SendCoinsRecipient *out)
{
    // Convert campuscash:// to CampusCash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("campuscash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "CampusCash:");
    }
    QUrl uriInstance(uri);
    return parseCampusCashURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CampusCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CampusCash.lnk
...
{
    return GetAutostartDir() / "CampusCash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a CampusCash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CampusCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the CampusCash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add CampusCash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CampusCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  CampusCash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CampusCash-Qt"));
