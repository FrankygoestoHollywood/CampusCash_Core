...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", CampusCashUnits::CCASH);
    ...
    if (!settings.contains("nAnonymizeCampusCashAmount"))
        settings.setValue("nAnonymizeCampusCashAmount", 1000);
    nAnonymizeCampusCashAmount = settings.value("nAnonymizeCampusCashAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCampusCashAmount"))
        SoftSetArg("-anonymizeCampusCashamount", settings.value("nAnonymizeCampusCashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCampusCashAmount:
            return QVariant(nAnonymizeCampusCashAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeCampusCashAmount:
            nAnonymizeCampusCashAmount = value.toInt();
            settings.setValue("nAnonymizeCampusCashAmount", nAnonymizeCampusCashAmount);
            emit AnonymizeCampusCashAmountChanged(nAnonymizeCampusCashAmount);
            break;
        default:
            break;
        }
