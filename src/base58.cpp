...
namespace {
    class CCampusCashAddressVisitor : public boost::static_visitor<bool> {
    private:
        CCampusCashAddress *addr;
    public:
        CCampusCashAddressVisitor(CCampusCashAddress *addrIn) : addr(addrIn) { }
...
bool CCampusCashAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CCampusCashAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CCampusCashAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CCampusCashAddressVisitor(this), dest);
}

bool CCampusCashAddress::IsValid() const {
...
CTxDestination CCampusCashAddress::Get() const {
    if (!IsValid())
...
bool CCampusCashAddress::GetKeyID(CKeyID &keyID) const {
...
bool CCampusCashAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CCampusCashSecret::SetKey(const CKey& vchSecret) {
...
CKey CCampusCashSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CCampusCashSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CCampusCashSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CCampusCashSecret::SetString(const std::string& strSecret) {
